...
export function init() {
    ...
    if (addForm) {
        ...
        // Actually, with ViewTransitions, the whole DOM is replaced, so the element is new.
        // We don't need to worry about duplicate listeners on the same element.
    ...
    if (listContainer) {
        // Use a new listener each time, but since listContainer is new, it works.
    ...
    // We can return cleanup function if needed, but on ViewTransitions, the whole page is destroyed/replaced?
    // Not exactly, scripts might linger? No, regular scripts in .astro run once.
